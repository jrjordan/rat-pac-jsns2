...
GLG4PMTOpticalModel::DoIt(const G4FastTrack& fastTrack, G4FastStep& fastStep)
{
  ...
  //Figure out which process created the photon. Added by J. Jordan
  origin_flag = -1;
  if ( fastTrack.GetPrimaryTrack()->GetCreatorProcess() ) {
    if ( fastTrack.GetPrimaryTrack()->GetCreatorProcess()->GetProcessName()=="Cerenkov" )
      origin_flag = 0;
    else if ( fastTrack.GetPrimaryTrack()->GetCreatorProcess()->GetProcessName()=="Scintillation" )
      origin_flag = 1;
    else if ( fastTrack.GetPrimaryTrack()->GetCreatorProcess()->GetProcessName()=="Reemission" )
      origin_flag = 2;
    else
      origin_flag = -1;
  }
